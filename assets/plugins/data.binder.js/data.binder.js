...
(function ($, document, window, undefined) {
  ...
  var dataBinder = function (that, element, path) {
    ...
    for (var i = 0, l = parts.length; i < l; i++) {
      ...
      var $m = $segments[0];
      var $arg = $segments[1]
        ? $.map($segments[1].replace(/\]/g, "").split(/\[/), $.trim)
        : [""];
      var $f = $arg.shift();
  ...
  dataBinder.prototype = {
    ...
    event: function (event) {
      ...
      if (this._custom_events(event)) is_updated = true;

      if (is_updated && !this.element.is("[data-bind-no-trigger]"))
        this.that._update();
    },
    ...
    _add_method: function (type, func, arg) {
      this.methods[type] = [
        typeof this.that.opts[type][func] == "function" ? func : "none",
        arg,
      ];
    },
    ...
    _run: function () {
      ...
      return this.that.opts[type][this.methods[type][0]].apply(
        this,
        args.concat(this.methods[type][1]),
      );
    },
  };
  ...
  $[pluginName] = function (parent, options) {
    ...
    self.opts.input = {
      ...
      radio: function (element) {
        return $(
          "input:radio[name ='" + element.attr("name") + "']:checked",
        ).val();
      },
    ...
    self.opts.filterout = {
      ...
      currency: function (value) {
        var res = self.formatMoney(
          value,
          self.opts.money.decimals,
          self.opts.money.separator,
          self.opts.money.thousands,
        );
        if (self.opts.money.cutzero)
          res = res.replace(
            new RegExp("\\" + self.opts.money.separator + "0+$"),
            "",
          );
        return res;
      },
  ...
  $[pluginName].prototype = {
    ...
    _data_set: function getValueAt(keys, value) {
      ...
      while (i < len - 1) {
        if (typeof object[keys[i]] == "undefined") object[keys[i]] = {};
    ...
    _data_get: function getValueAt(keys, def) {
      ...
      if (typeof object[keys[i]] == "undefined") return def;
